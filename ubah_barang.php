...
<?php
    ...
    <form action="proses_ubah_barang.php" method="post">
        ...
        Nama Barang :
        <input type="text" name="nama" value="<?=$dt_barang['nama']?>" class="form-control">
        Tanggal Produksi :
        <input type="date" name="tanggal_produksi" value="<?=$dt_barang['tanggal_produksi']?>" class="form-control">
        Tempat Produksi :
        <textarea name="tempat_produksi" class="form-control" rows="4"><?=$dt_barang['tempat_produksi']?></textarea>
        Kode Barang :
        <select name="kode_barang" class="form-control">
            <option></option>
<?php
            include "koneksi.php";
            $qry_jenis_barang=mysqli_query($conn,"select * from jenis_barang");
            while ($data_jenis_barang=mysqli_fetch_array($qry_jenis_barang)) {
                if ($data_jenis_barang['kode_barang']==$dt_barang['kode_barang']) {
                    $selek="selected";
                } else {
                    $selek="";
                }
            echo'<option value="'.$data_jenis_barang['kode_barang'].'"'.$selek.'>'.$data_jenis_barang['jenis_barang'].'</option>';
            }
            ?>
             </select>
        <input type="submit" name="simpan" value="Ubah data barang" class="btn btn-primary">
    </form>
 </main>
